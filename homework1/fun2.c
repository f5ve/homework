...
struct arg operate(int* array, int size)
{
	struct arg res;
	res.sum = usum(array, size);
	res.avg = ((float)res.sum) / ((float)size);
...
void sysbsort(int *array, int n)
{
	struct timeval tv1, tv2;

	gettimeofday(&tv1, NULL);
	bsort((void*)array, n, sizeof(int), compar);
	gettimeofday(&tv2, NULL);
	printf("bsort spend %ld us:\n", tv2.tv_usec - tv1.tv_usec);
}

void sysqsort(int *array, int n)
{
	struct timeval tv1, tv2;

	gettimeofday(&tv1, NULL);
	qsort((void*)array, n, sizeof(int), compar);
	gettimeofday(&tv2, NULL);
